...
export type TabContent = TreeItem & { readOnly?: boolean }
const treeStore = useTreeStore();
const arrOfObjects = ref<TabContent[]>([])
const tab = ref<number>(0)

watchEffect(() => {
  if (treeStore.selectedObject !== null) {
    const existing = arrOfObjects.value.find(item => item.id === treeStore.selectedObject.id)
    if (existing) {
      const newObj = {...existing, readOnly: true}
